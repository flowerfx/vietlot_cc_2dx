...
namespace RKUtils
{
static size_t s_RKThread_MainThreadID = 0;
...
void* _NamedThreadWrapperFunction(void* pThreadInternalVoidPtr)
{
  RKThreadInternal* pThreadInternal = (RKThreadInternal*)pThreadInternalVoidPtr;
  pthread_setname_np(pThreadInternal->name.GetString());
  return pThreadInternal->pThreadAddress(pThreadInternal->pThreadParams);
}
...
#ifndef RKRETAIL
  int iResult = pthread_create(&pThreadInternal->handle, &attr, _NamedThreadWrapperFunction, pThreadInternal);
#else
  int iResult = pthread_create(&pThreadInternal->handle, &attr, pThreadInternal->pThreadAddress, pThreadInternal->pThreadParams);
#endif
...
void RKThread_Sleep(uint32 seconds, uint32 milliseconds)
{
  ...
  sleepTime.tv_nsec = milliseconds * 100000;
  if(sleepTime.tv_nsec > 99999999)
  {
      sleepTime.tv_nsec = 99999999;
  }
...
{
  RKThreadInternal* pThreadInternal = (RKThreadInternal*)pThread;
  auto id =(size_t)pThreadInternal->handle;
    return id;
}
...
{  
  return pthread_cond_signal(&pCondition->condition) == 0;
}
...
{  
  return pthread_cond_broadcast(&pCondition->condition) == 0;
}

///----------------------------------------------------------------------
///----------------------------------------------------------------------
bool RKThreadCondition_Sleep(RKThreadCondition* pCondition, RKCriticalSection* pMutex, DWORD time_sleep)
{  
  RKCriticalSectionInternal* pCriticalSectionInternal = (RKCriticalSectionInternal*)pMutex;
    int iRetVal = 0;
    if(time_sleep >= 0xFFFFFF)
    {
         iRetVal = pthread_cond_wait(&pCondition->condition, &pCriticalSectionInternal->mutex);
    }
    else{
        timespec time_spec;
        time_spec.tv_nsec = (float)time_sleep / 1000;
        time_spec.tv_nsec = time_sleep * 1000*1000;
        iRetVal = pthread_cond_timedwait(&pCondition->condition, &pCriticalSectionInternal->mutex, &time_spec);
    }
