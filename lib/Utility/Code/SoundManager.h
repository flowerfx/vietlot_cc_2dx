...
namespace Utility
{
	namespace Sound
	{
		...
		private:
			...
			struct SoundQueue
			{
				...
				SoundQueue(const RKString & id, const RKString & p)
				{
					ID = id;
					path = p;
				}
			...
			std::vector<SoundQueue> p_queue_id_sound;

			void PlayInternalSound(const char * sound_path, const char * ID);
#endif
		public:
			...
			//
			void PlaySoundEffect(const RKString & ID_SOUND, float delay = 0, int loop = 0,bool imediately = false);
			void PlayMusicEffect(const RKString & ID_MUSIC, bool loop = true, bool use_in_thread = true , const std::function<void(void)> & call_back = nullptr);
			//
	...
	#define PLAY_SOUND(x) GetSound->PlaySoundEffect(x)
	#define PLAY_MUSIC(x) GetSound->PlayMusicEffect(x)
}
#ifndef USE_NEW_SOUND
