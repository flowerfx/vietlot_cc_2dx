...
namespace Utility
{
	...
	namespace xml
	{
		...
		public:
			...
			//remove 
			bool RemoveDataInt(std::string name);
			bool RemoveDatafloat(std::string name);
			bool RemoveDataChar(std::string name);
			bool RemoveDataVector2(std::string name);
			bool RemoveDataVector3(std::string name);
			bool RemoveDataColor(std::string name);
			...
			//append from other
			void Append(BasicDec * data, bool overwrite = false);
		...
		*/
		...
		// UIWIdget component dec xml
		struct UIWidgetComDec
		{
			RKString path_xml_com;
		public:
			UIWidgetComDec();
			UIWidgetComDec(RKString name);
			virtual ~UIWidgetComDec();

			void InitValue();
		};

		//load template value (common / template / origin value/ first value/ command)
		struct UIWidgetTemplate
		{
			RKString name_template;
			BasicDec * OriginValue;
			BasicDec * FirstValue;
			BasicDec * GeneralValue;
			std::map<std::string, std::string> p_list_command;

			//qchien todo: 26/10/2016 the template not support child widget, child template, multi template but the future i will support its

		public:
			UIWidgetTemplate();
			virtual ~UIWidgetTemplate();
			void InitValue();
		...
		*/
		...
		private:
			...
			std::map<RKString, UIWidgetTemplate*>	p_ListTemplate;

			std::map<RKString, UIWidgetComDec*>		p_ListWidgetCom;
		...
		public:
			...
			*/
			...
			//support com
			UILayerWidgetDec * OnParseWidgetEntity1(TiXmlElement * element_xml, UILayerWidgetDec* parent_widget, UIMenuWidgetDec * UIDec, sound_click_dec * global_id_sound_click);

			TYPE_UI_WIDGET GetUIWidget(RKString name);
			/*
			-----------------------------------------------------
			*/
			UIWidgetTemplate * GetUIWidgetTemplateByName(RKString str);
			void InsertUIWidgetTemplate(RKString str, UIWidgetTemplate * t);
			void OnLoadWidgetTemplateDecXML(TiXmlDocument * objectXMl);

			void OnLoadTemplateDetailXML(TiXmlElement * element_xml, Vec2 design_size = Vec2(1,1));

			void OnParseTemplateIntoWidgetByName(UILayerWidgetDec * widget, RKString name);
			void OnParseTemplateIntoWidget(UILayerWidgetDec * widget, UIWidgetTemplate * wtemplate);

			void OnParseTemplateIntoTemplateByName(UIWidgetTemplate * temp, RKString name);
			void OnParseTemplateIntoTemplate(UIWidgetTemplate * temp, UIWidgetTemplate * wtemplate);

			/*
			-----------------------------------------------------
			*/
			UIWidgetComDec * GetUIWidgetComByName(RKString str);
			void InsertUIWidgetCom(RKString str, UIWidgetComDec * t);
			void OnLoadUIWidgetComXML(TiXmlDocument * objectXMl);

			void CustomLoadXML(TiXmlDocument * objectXMl);
			/*
			-----------------------------------------------------
			*/
