...
#pragma region basic_dec_xml
		...
		//remove 
		bool BasicDec::RemoveDataInt(std::string name)
		{
			if (__name_int_.size() > 0 && __name_int_.contain(name))
			{
				__name_int_.erase(name);
		...
		bool BasicDec::RemoveDatafloat(std::string name)
		{
			if (__name_float_.size() > 0 && __name_float_.contain(name))
			{
				__name_float_.erase(name);
		...
		bool BasicDec::RemoveDataChar(std::string name)
		{
			if (__name_string_.size() > 0 && __name_string_.contain(name))
			{
				__name_string_.erase(name);
		...
		bool BasicDec::RemoveDataVector2(std::string name)
		{
			if (__name_vector_.size() > 0 && __name_vector_.contain(name))
			{
				__name_vector_.erase(name);
		...
		bool BasicDec::RemoveDataVector3(std::string name)
		{
			if (__name_vector3_.size() > 0 && __name_vector3_.contain(name))
			{
				__name_vector3_.erase(name);
		...
		bool BasicDec::RemoveDataColor(std::string name)
		{
			if (__name_color_.size() > 0 && __name_color_.contain(name))
			{
				__name_color_.erase(name);
		...
		//append from other
		void BasicDec::Append(BasicDec * data, bool overwrite /*= false*/)
		{
			//append color
			for (int i = 0; i < data->__name_color_.size(); i++)
			{
				auto value = data->__name_color_.atIdx(i);

				if (overwrite)
				{
					if (__name_color_.contain(value.value_first))
					{
						__name_color_.erase(value.value_first);
					}
					__name_color_.insert(value);
				}
				else
				{
					if (!__name_color_.contain(value.value_first))
			...
			//append vec2
			for (int i = 0; i < data->__name_vector_.size(); i++)
			{
				auto value = data->__name_vector_.atIdx(i);

				if (overwrite)
				{
					if (__name_vector_.contain(value.value_first))
					{
						__name_vector_.erase(value.value_first);
					}
					__name_vector_.insert(value);
				}
				else
				{
					if (!__name_vector_.contain(value.value_first))
			...
			//append vec3
			for (int i = 0; i < data->__name_vector3_.size(); i++)
			{
				auto value = data->__name_vector3_.atIdx(i);

				if (overwrite)
				{
					if (__name_vector3_.contain(value.value_first))
					{
						__name_vector3_.erase(value.value_first);
					}
					__name_vector3_.insert(value);
				}
				else
				{
					if (!__name_vector3_.contain(value.value_first))
			...
			//append string
			for (int i = 0; i < data->__name_string_.size(); i++)
			{
				auto value = data->__name_string_.atIdx(i);

				if (overwrite)
				{
					if (__name_string_.contain(value.value_first))
					{
						__name_string_.erase(value.value_first);
					}
					__name_string_.insert(value);
				}
				else
				{
					if (!__name_string_.contain(value.value_first))
			...
			//append int
			for (int i = 0; i < data->__name_int_.size(); i++)
			{
				auto value = data->__name_int_.atIdx(i);

				if (overwrite)
				{
					if (__name_int_.contain(value.value_first))
					{
						__name_int_.erase(value.value_first);
					}
					__name_int_.insert(value);
				}
				else
				{
					if (!__name_int_.contain(value.value_first))
			...
			//append float
			for (int i = 0; i < data->__name_float_.size(); i++)
			{
				auto value = data->__name_float_.atIdx(i);

				if (overwrite)
				{
					if (__name_float_.contain(value.value_first))
					{
						__name_float_.erase(value.value_first);
					}
					__name_float_.insert(value);
				}
				else
				{
					if (!__name_float_.contain(value.value_first))
...
#pragma region basic_w_ui_dec_xml
		...
		UIWidgetComDec::UIWidgetComDec()
		{
			
		}
		UIWidgetComDec::UIWidgetComDec(RKString name)
		{
			path_xml_com = name;
		}
		UIWidgetComDec::~UIWidgetComDec()
		...
		void UIWidgetComDec::InitValue()
		{

		}

		UIWidgetTemplate::UIWidgetTemplate()
		{
			name_template = "";
			OriginValue = nullptr;
			FirstValue = nullptr;
			GeneralValue = nullptr;
			p_list_command.clear();
		}
		...
		void UIWidgetTemplate::InitValue()
		{
			OriginValue = new BasicDec();
			FirstValue = new BasicDec();
...
#pragma endregion basic_w_ui_dec_xml
		...
		*/
		...
		LoadDataXML::LoadDataXML()
		{
			...
			p_ListTemplate.clear();
			p_ListWidgetCom.clear();
		}
		...
		LoadDataXML::~LoadDataXML()
		{
			...
			for (auto it : p_ListTemplate)
			{
				delete it.second;
			}
			p_ListTemplate.clear();

			for (auto it : p_ListWidgetCom)
			{
				delete it.second;
			}
			p_ListWidgetCom.clear();
		...
		void LoadDataXML::OnLoadXML1(std::function<void(TiXmlDocument*)> func, RKString path_xml)
		{
			...
			if (p_IsUseArkXML)
			{
				tinyxml2::XMLError error = p_ObjectXML->LoadFileZip((DataDir() + TargetXML).c_str());
...
#pragma endregion load_menu_widget_dec
					...
					if (name_table == rootUI->Attribute("name"))
					{
						...
						do
						{
							if (UIMenuWidget)
							{
								...
								if (p_IsUseArkXML)
								{
									tinyxml2::XMLError error = p_ObjectXML->LoadFileZip((DataDir() + std::string(path)).c_str());
									if (error == tinyxml2::XMLError::XML_NO_ERROR)
									{
		...
		bool LoadDataXML::OnParseValueIntoXMLDec(RKString name_common_value, RKString common_value, xml::BasicDec * data, xml::UILayerWidgetDec * _widget)
		{
			...
			else
			{
				...
				if (IsStringValue)
				{
					...
					data->ReplaceDataChar(name_common_value.GetString(), (char*)str_value.GetString());
				}
			}
		...
		UILayerWidgetDec * LoadDataXML::OnParseWidgetEntity1(TiXmlElement * element_xml, UILayerWidgetDec* _widget, UIMenuWidgetDec * UIDec, sound_click_dec * global_id_sound_click)
		{
			//parse the child widget into its parent

			UILayerWidgetDec * _child_widget_dec = nullptr;
			if (element_xml)
			{
					//check is widget com
					RKString path_com = "";
					if (element_xml->Attribute("com"))
					{
						path_com = element_xml->Attribute("com");
					}

					if (path_com != "")
					{
						auto real_path = GetUIWidgetComByName(path_com);
						if (real_path)
						{
							OnLoadXML1([_widget , this, UIDec, global_id_sound_click, &_child_widget_dec](TiXmlDocument * p_ObjectXML) {
								auto Root_tmp = p_ObjectXML->FirstChildElement("GAME");
								if (!Root_tmp)
								{
									PASSERT2(false, "XML MenuWidget fail to load!");
								}
								...
								{
									auto w_com_widget = Root_tmp->FirstChildElement("Widget");
									_child_widget_dec = this->OnParseWidgetEntity(w_com_widget, _widget, UIDec, global_id_sound_click);
								}
							}, real_path->path_xml_com);
						}
						else
						{
							PWARN1("ERROR ! CANNOT FINT PATH WIDGET COM");
						}

					}
					else
					{
						_child_widget_dec = OnParseWidgetEntity(element_xml, _widget, UIDec, global_id_sound_click);
					}
			}
			return _child_widget_dec;
		}
		...
		UILayerWidgetDec * LoadDataXML::OnParseWidgetEntity(TiXmlElement * element_xml, UILayerWidgetDec* parent_widget, UIMenuWidgetDec * UIDec, sound_click_dec * global_id_sound_click)
		{
			...
			//template value
			auto template_value = element_xml->FirstChildElement("template_value");
			if (template_value)
			{
				auto template_obj = template_value->FirstChildElement("template");
				do
				{
					if (template_obj)
					{
						RKString name_template = template_obj->Attribute("name");

						OnParseTemplateIntoWidgetByName(_widget, name_template);

						template_obj = template_obj->NextSiblingElement();
					}
				} while (template_obj);
			}
			...
			if (!common_value)
			{
//				PWARN1("ERROR: the common value was not found !");
			}
			...
			if (child_element)
			{
				...
				if (!str_pos)
				{
					PWARN1("ERROR: NOT FOUND: origin position element");
				}
			...
			{
				PWARN1("ERROR: NOT FOUND: position element");
			}
			...
			if (child_element)
			{
				...
				if (!str_value)
				{
					PWARN1("ERROR: NOT FOUND: origin size element");
				}
			...
			if (child_element)
			{
				...
				if (str_value)
				{
					...
					_widget->OriginValue->ReplaceDataColor("tint", Color4B(atoi(color_tint[0].GetString()), atoi(color_tint[1].GetString()), atoi(color_tint[2].GetString()), 255));
				}
			}
	...
	//					contain_cmd.ReplaceAll("\n", "");
			...
			do
			{
				if (_child_widget)
				{
					...
					//check is widget com
					RKString path_com = "";
					if (_child_widget->Attribute("com"))
					{
						path_com = _child_widget->Attribute("com");
					}
					...
					if (path_com != "")
					{
						...
						if (real_path)
						{
							OnLoadXML1([_widget, this, UIDec, global_id_sound_click](TiXmlDocument * p_ObjectXML) {
								auto Root_tmp = p_ObjectXML->FirstChildElement("GAME");
								if (!Root_tmp)
								{
									PASSERT2(false, "XML MenuWidget fail to load!");
								...
								{
									auto w_com_widget = Root_tmp->FirstChildElement("Widget");
									auto _child_widget_dec = this->OnParseWidgetEntity(w_com_widget, _widget, UIDec, global_id_sound_click);
					...
					{
						_child_widget_dec = OnParseWidgetEntity(_child_widget, _widget, UIDec, global_id_sound_click);
						_widget->InsertChildWidget(_child_widget_dec);
					}
				...
				if (common_value)
				{
					...
					do
					{
						if (common_obj)
						{
							...
							bool res = OnParseValueIntoXMLDec(name_common_value, common_value, _object->CommonValue, nullptr);
							common_obj = common_obj->NextSiblingElement();
						}
					} while (common_obj);
				}
		...
		*/
		...
		UIWidgetTemplate * LoadDataXML::GetUIWidgetTemplateByName(RKString str)
		{
			if (p_ListTemplate.find(str) != p_ListTemplate.end())
			{
				return p_ListTemplate.at(str);
			}
			return nullptr;
		}

		void LoadDataXML::InsertUIWidgetTemplate(RKString str, UIWidgetTemplate * t)
		{
			p_ListTemplate.insert({ str , t });
		}

		void LoadDataXML::OnLoadTemplateDetailXML(TiXmlElement * wtemplate, Vec2 design_size)
		{
			if (wtemplate)
			{
				UIWidgetTemplate * w_template = new UIWidgetTemplate();
				w_template->InitValue();
				RKString name_template = wtemplate->Attribute("name");
				w_template->name_template = name_template;

				//template in template

				//template value
				auto template_value = wtemplate->FirstChildElement("template_value");
				if (template_value)
				{
					auto template_obj = template_value->FirstChildElement("template");
					do
					{
						if (template_obj)
						{
							...
							OnParseTemplateIntoTemplateByName(w_template, name_template);
				...
				//common_value
				auto common_value = wtemplate->FirstChildElement("common_value");
					...
					do
					{
						if (common_obj)
						{
							...
							if (name_common_value == "source" || name_common_value == "title" || name_common_value == "cap_insert")
							{
								...
								w_template->GeneralValue->ReplaceDataChar(name_common_value.GetString(), (char*)common_value.GetString());
							}
							else
							{
								bool res = OnParseValueIntoXMLDec(name_common_value, common_value, w_template->GeneralValue, nullptr);
							}
							common_obj = common_obj->NextSiblingElement();
						}
					} while (common_obj);
				...
				//parse position
				auto child_element = wtemplate->FirstChildElement("pos");
				if (child_element)
				{
					const char * str_pos = child_element->Attribute("origin");
					if (!str_pos)
					...
					origin_value = Vec2(pos.x / design_size.x, pos.y / design_size.y);
					first_value = origin_value;
					w_template->OriginValue->ReplaceDataVector2("pos_ratio", origin_value);
					...
					if (str_pos)
					{
						pos = ConvertToVector2(str_pos);
						first_value = Vec2(pos.x / design_size.x, pos.y / design_size.y);
						w_template->FirstValue->InsertDataVector2("pos_ratio", first_value);
					}
				}
				...
				if (child_element)
				{
					...
					if (w_template->GeneralValue->GetDataInt("stick_screen") == 1)
						origin_value = Vec2(size.x / design_size.x, size.y / design_size.y);
					else
						origin_value = size;
					first_value = origin_value;
					...
					w_template->OriginValue->ReplaceDataVector2("scale_ratio", origin_value);
					//
					str_value = child_element->Attribute("first");
					if (str_value)
					{
						...
						w_template->FirstValue->InsertDataVector2("scale_ratio", first_value);
					}
				}


				//parse opacity
				child_element = wtemplate->FirstChildElement("opacity");
				if (child_element)
				{
					const char * str_value = child_element->Attribute("origin");
					...
					w_template->OriginValue->ReplaceDataInt("opacity", origin_value);
					//_widget->FirstValue->ReplaceDataInt("opacity", first_value);
					//
					str_value = child_element->Attribute("first");
					if (str_value)
					{
						first_value = atoi(str_value);
						w_template->FirstValue->InsertDataInt("opacity", first_value);
					}
				}

				//parse rotate
				child_element = wtemplate->FirstChildElement("rotate");
				if (child_element)
				{
					const char * str_value = child_element->Attribute("origin");
					...
					w_template->OriginValue->ReplaceDatafloat("rotate", origin_value);
					//_widget->FirstValue->ReplaceDatafloat("rotate", first_value);
					//
					str_value = child_element->Attribute("first");
					if (str_value)
					{
						first_value = atof(str_value);
						w_template->FirstValue->InsertDatafloat("rotate", first_value);
					}
				...
				if (child_element)
				{
					...
					{
						auto color_tint = RKString(str_value).Split("_");
						w_template->OriginValue->ReplaceDataColor("tint", Color4B(atoi(color_tint[0].GetString()), atoi(color_tint[1].GetString()), atoi(color_tint[2].GetString()), 255));
					}
				}

				//parse command

				child_element = wtemplate->FirstChildElement("COMMAND");
				if (child_element)
				{
					auto command_xml = child_element->FirstChildElement("cmd");
					do
					{
						if (command_xml)
						{
							...
							w_template->p_list_command.insert({ name_cmd, contain_cmd });
							command_xml = command_xml->NextSiblingElement();
						}
					} while (command_xml);
				}

				InsertUIWidgetTemplate(name_template, w_template);

				wtemplate = wtemplate->NextSiblingElement();
			}
		}

		void LoadDataXML::OnLoadWidgetTemplateDecXML(TiXmlDocument * objectXMl)
		{
			auto Root = objectXMl->FirstChildElement("GAME");
			if (!Root)
			{
			...
			auto default_template = Root->FirstChildElement("default_template");
			if (default_template)
			{
				auto wtemplate = default_template->FirstChildElement("template");
				do
				{
					if (wtemplate)
					{
						OnLoadTemplateDetailXML(wtemplate);
						wtemplate = wtemplate->NextSiblingElement();
					}
			...
			Vec2 screen_size = GetGameSize();
			auto list_template = Root->FirstChildElement("template_list");
			do
			{
				if (list_template)
				{
					Vec2 _rg_size_x = ConvertToVector2(list_template->Attribute("range_size_x"));
					bool IsFitForThisDesign = false;
					//check the screen in the range of the design size
					if (screen_size.x >= _rg_size_x.x && (screen_size.x <= _rg_size_x.y || _rg_size_x.y <= -1))
					{
					...
					if (IsFitForThisDesign)
					{
						Vec2 design_size = ConvertToVector2(list_template->Attribute("design_size"));
						auto wtemplate = list_template->FirstChildElement("template");
						do
						{
							if (wtemplate)
							{
								OnLoadTemplateDetailXML(wtemplate, design_size);
								wtemplate = wtemplate->NextSiblingElement();
							}
					...
					list_template = list_template->NextSiblingElement("template_list");
				}
			} while (list_template);

		}

		void LoadDataXML::OnParseTemplateIntoWidgetByName(UILayerWidgetDec * widget, RKString name)
		{
			auto w_temp = GetUIWidgetTemplateByName(name);
			if (w_temp)
			{
				OnParseTemplateIntoWidget(widget, w_temp);
			}
		}

		void LoadDataXML::OnParseTemplateIntoWidget(UILayerWidgetDec * widget, UIWidgetTemplate * wtemplate)
		{
			if (!widget || !wtemplate)
				return;

			widget->OriginValue->Append(wtemplate->OriginValue, true);
			widget->FirstValue->Append(wtemplate->FirstValue, true);
			widget->GeneralValue->Append(wtemplate->GeneralValue, true);

			for (auto it : wtemplate->p_list_command )
			{
				if (widget->p_list_command.find(it.first) != widget->p_list_command.end())
				{
					widget->p_list_command.erase(it.first);
				}
				widget->p_list_command.insert(it);
			}
		}

		void LoadDataXML::OnParseTemplateIntoTemplateByName(UIWidgetTemplate * temp, RKString name)
		{
			auto w_temp = GetUIWidgetTemplateByName(name);
			if (w_temp)
			{
				OnParseTemplateIntoTemplate(temp, w_temp);
			}
		}

		void LoadDataXML::OnParseTemplateIntoTemplate(UIWidgetTemplate * temp, UIWidgetTemplate * wtemplate)
		{
			if (!temp || !wtemplate)
				return;

			temp->OriginValue->Append(wtemplate->OriginValue, true);
			temp->FirstValue->Append(wtemplate->FirstValue, true);
			temp->GeneralValue->Append(wtemplate->GeneralValue, true);

			for (auto it : wtemplate->p_list_command)
			{
				if (temp->p_list_command.find(it.first) != temp->p_list_command.end())
				{
					temp->p_list_command.erase(it.first);
				}
				temp->p_list_command.insert(it);
			}
		}


		UIWidgetComDec * LoadDataXML::GetUIWidgetComByName(RKString str)
		{
			if (p_ListWidgetCom.find(str) != p_ListWidgetCom.end())
			{
				return p_ListWidgetCom.at(str);
			}
			return nullptr;
		}
		
		void LoadDataXML::InsertUIWidgetCom(RKString str, UIWidgetComDec * t)
		{
			p_ListWidgetCom.insert({ str , t });
		}
		
		void LoadDataXML::OnLoadUIWidgetComXML(TiXmlDocument * objectXMl)
		{
			auto Root = objectXMl->FirstChildElement("GAME");
			if (!Root)
			{
			...
			auto list_widget = Root->FirstChildElement("template_widget_dec");
			auto w = list_widget->FirstChildElement("widget");
			do
			{
				if (w)
				{

					RKString name = w->Attribute("name");
					RKString path = w->Attribute("value");
					InsertUIWidgetCom(name, new xml::UIWidgetComDec(path));
		...
		void LoadDataXML::CustomLoadXML(TiXmlDocument * objectXMl)
		{
