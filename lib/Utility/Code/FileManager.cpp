...
#include "RKFile_Code/RKFile.h"
	...
	namespace File
	{
		...
		FILE * GetLocalFile(const RKString &  namepath, const RKString &  sub)
		{
			...
			RKString finalPath = RKString(p_gLocalPath) + "/" + npath;
		...
		const RKString ResolveNamePath(const RKString & namepath)
		{
			...
			if (p_gInstallPath != "")
			{
				finalPath = RKString(p_gInstallPath) + DataDir() + npath;
			}
			else
			{
#if defined WP8 || defined OS_W8 || defined OS_W10
				...
				p_gInstallPath = wstos(InstallPath->Data());
#elif defined OS_IOS
                auto ios_namepath = DataDir() + npath;
				auto fullPath = GetFullPathResourceFile_IOS(ios_namepath.GetString());
				return fullPath;
#endif
				finalPath = RKString(p_gInstallPath) + DataDir() + npath;
			}
		...
		const char * GetDataFromFile(const RKString &  namepath)
		{
			...
			if (!fp)
			{
				...
				if (!fp)
				{
					...
					long lSize = 0;
					void * data = nullptr;
					auto res = cocos2d::FileUtils::getInstance()->GetDataFromZipFile((DataDir() + namepath).GetString(), data, &lSize);
					if (res)
					{
						char * buffer = nullptr;
						// allocate memory to contain the whole file:
			...
			return GetDataFromHandleFile(fp);
		}
		///////////////////////////////////////////////////////////////
		DataSave::DataSave() 
		{
		...
		bool DataSave::GetFloatByName(const RKString &  name, float & result)
		{
		...
		void DataSave::SetFloatByName(const RKString &  name, float data)
		{
			if (p_data_save->HaveDatafloat(name.GetString()))
			{
		...
		bool DataSave::RemoveFloatByName(const RKString &  name)
		{
			return p_data_save->RemoveDatafloat(name.GetString());
		}

		bool DataSave::GetCharByName(const RKString &  name, RKString & result)
		{
		...
		void DataSave::SetCharByName(const RKString &  name, const RKString &  data)
		{
			if (p_data_save->HaveDataChar(name.GetString()))
			{
		...
		bool DataSave::RemoveCharByName(const RKString &  name)
		{
			return p_data_save->RemoveDataChar(name.GetString());
		}

		bool DataSave::GetIntByName(const RKString &  name, int & result)
		{
		...
		void DataSave::SetIntByName(const RKString &  name, int data)
		{
			if (p_data_save->HaveDataInt(name.GetString()))
			{
		...
		bool DataSave::RemoveIntByName(const RKString &  name)
		{
			return p_data_save->RemoveDataInt(name.GetString());
		}
		...
		void SaveManager::SaveTheGameSave(const RKString &  name_save)
		{
#if defined OS_W8 || defined WP8 || defined OS_IOS || defined OS_ANDROID_ || defined _WIN32 || defined OS_MAC || defined OS_W10
			FILE * f = GetLocalFile(name_save, "wb");
#else
			FILE * f = GetFile(name_save, "wb");
#endif
			if (!f)
			{
		...
		void SaveManager::LoadTheGameSave(const RKString &  name_save)
		{
			auto str = OnLoadDataSave(name_save);
			if (str != "")
			{
		...
		const RKString   SaveManager::OnLoadDataSave(const RKString &  name_save)
		{
			int file_exist = 1;
#if defined OS_W8 || defined WP8 || defined OS_IOS || defined OS_ANDROID_ || defined _WIN32 || defined OS_MAC || defined OS_W10
			FILE * f = GetLocalFile(name_save, "rb"); //read the file in local path
#else
			FILE * f = GetFile(name_save, "rb");
#endif
			if (!f) //the file not exist mean that the file is remove or this this the first game play
			{
				...
				f = GetLocalFile(name_save, "wb"); //create a new file
#else
				f = GetFile(name_save, "wb");
#endif
				file_exist = 0;
			}
			if (!f)
			...
			{
				const char * buffer_file = GetDataFromHandleFile(f);
				if (buffer_file)
				{
		...
		const RKString   SaveManager::OnEngryptString(const RKString &  in)
		{
			std::string out = "";
			int idx = 0;
		...
		const RKString   SaveManager::OnDegryptString(const RKString &  in)
		{
			std::string out = "";
			int idx = 0;
			std::string t_in = in.GetString();
		...
		const RKString   SaveManager::CreateBufferSave(bool Reset)
		{
			OnGenerateUserName();
			//
		...
		bool SaveManager::OnExtractData(const RKString &  str)
		{
			bool res = true;
		...
		const RKString &  SaveManager::GetUDID()
		{
		...
		bool SaveManager::LoadLevelFromBufferData(const RKString &  str)
		{
			bool res = true;
