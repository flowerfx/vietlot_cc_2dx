...
namespace Utility
{
	namespace Sound
	{
		...
		//
#ifdef USE_NEW_SOUND
		void SoundManager::PlayInternalSound(const char * sound_path, const char * ID)
		{
			//if (_current_audio_id == AudioEngine::INVALID_AUDIO_ID)
			{
				_current_audio_id = AudioEngine::play2d(sound_path, false, _volume_sound, nullptr);
				PWARN1("[SOUND MANAGER] : ID: %s PLAY", ID);
				if (_current_audio_id != AudioEngine::INVALID_AUDIO_ID)
				{
					AudioEngine::setFinishCallback(_current_audio_id, [this](int id, const std::string& filePath)
					{
						_current_audio_id = AudioEngine::INVALID_AUDIO_ID;
						if (p_queue_id_sound.size() > 0)
						{
							cocos2d::Director::getInstance()->getScheduler()->performFunctionInCocosThread([this] {
								this->PlayInternalSound(p_queue_id_sound[0].path.GetString(), p_queue_id_sound[0].ID.GetString());
								p_queue_id_sound.erase(p_queue_id_sound.begin());
							});
						}
			...
			//else
			//{
			//	p_queue_id_sound.push_back(SoundQueue(ID, sound_path));
			//}
		}
#endif
		void SoundManager::PlaySoundEffect(const RKString & ID_SOUND, float delay, int loop, bool imediately)
		{
			if (!p_IsInitialized)
				return;
			...
			{
#ifdef USE_NEW_SOUND
//				this->StopTheCurrentSound();
//				this->p_queue_id_sound.clear();
#endif
			}
			RKString full_path = "";
...
#ifdef USE_NEW_SOUND
			PlayInternalSound(full_path.GetString(), ID_SOUND.GetString());
#else
			p_currentIdsoundfx = GetSimpleSound->playEffect(full_path.GetString() , false);
#endif
		}
		void SoundManager::PlayMusicEffect(const RKString & ID_MUSIC, bool loop, bool use_in_thread , const std::function<void(void)> & call_back)
		{
...
//			}
			...
			{
				_current_music_id = AudioEngine::play2d(full_path.GetString(), loop, _volume_music, nullptr);
				if (_current_music_id != AudioEngine::INVALID_AUDIO_ID)
				{
					_on_playing_music_id = _current_music_id;
					p_currentMusicPath = full_path.GetString();
					AudioEngine::setFinishCallback(_current_music_id, [& , call_back , ID_MUSIC](int id, const std::string& filePath)
					{
						_current_music_id = AudioEngine::INVALID_AUDIO_ID;
