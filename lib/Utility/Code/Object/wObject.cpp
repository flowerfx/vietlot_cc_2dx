...
namespace Utility
{
	namespace wObject
	{
		...
		{	
			if (_touchListener)
			{
				GetResource()->getEventDispatcher()->removeEventListener(_touchListener);
...
#pragma region init_method
		...
		{
			auto xml_dec = XMLMgr->GetWideObjectByName(name);

			bool res = OnInitWideObjectXML(xml_dec, UseAntiAlias);
		...
		bool wBasicObject::OnInitWideObjectXML(xml::Wide_ObjectDec * object_dec, bool UseAntiAlias)
		{
			...
			case 7:
			{
				...
				else if (object_dec->type_object == 6)
				{
					...
					Scale9Sprite * p = Scale9Sprite::createWithSpriteFrameName(object_dec->name_source.GetString(), sprite_frame->GetCapInsert());
		...
		bool wBasicObject::SetActionCommandWithName(RKString name, CallFunc * call_back, bool force)
		{
			...
			auto action_squence = action_command->GetSequenceActionData(
				p_action_process, 
				this->GetResource(), 
				this->p_parent_object ? 
				this->p_parent_object->GetResource() : nullptr, 
				flag,
				false);
...
#pragma region normal_method
		...
		void wBasicObject::SetSize(Vec2 Size)
		{
			...
			{
				int state_scale = p_xml_dec->CommonValue->GetDataInt("stage_scale9");
				if (state_scale == 0)
				{
					static_cast<Scale9Sprite*>(p_Resource)->setContentSize(cocos2d::Size(Size.x, Size.y));
				}
				else if (state_scale == 1)
				{
					Vec2 origin_size = GetResource()->getContentSize();
					float scale = Size.x / origin_size.x;

					static_cast<Scale9Sprite*>(p_Resource)->setContentSize(cocos2d::Size(origin_size.x, Size.y * (1 / scale)));
					GetResource()->setScale(scale);
				}
				else if (state_scale == 2)
				{
					Vec2 origin_size = GetResource()->getContentSize();
					float scale = Size.y / origin_size.y;

					static_cast<Scale9Sprite*>(p_Resource)->setContentSize(cocos2d::Size(Size.x * (1 / scale), origin_size.y));

					GetResource()->setScale(scale);

				}
				else if (state_scale == 3)
				{
					Vec2 own_design = p_xml_dec->CommonValue->GetDataVector2("design_scale9");
					if (own_design.x == 0)
					{
						own_design = this->design_size;
					}

					float delta_scalex = GetGameSize().width / (own_design.x);
					float delta_scaley = GetGameSize().height / (own_design.y);
					Vec2 scale_size_design = Vec2(Size.x / delta_scalex, Size.y / delta_scalex);

					static_cast<Scale9Sprite*>(p_Resource)->setContentSize(cocos2d::Size(scale_size_design.x, scale_size_design.y));

					GetResource()->setScale(delta_scalex);
				}
				else
				{
...
#pragma endregion normal_method
		...
		void		wBasicObject::OnTouchBeginObject()
		{

		}
		void		wBasicObject::OnTouchMoveObject()
		{

		}
		void		wBasicObject::OnTouchEndObject()
		{

		}
		void wBasicObject::onTouchesBegan(const std::vector<Touch*>& touches, Event *unused_event)
		{
			auto pos_start = touches[0]->getStartLocation();
			Vec2 node_pos = p_Resource->convertToNodeSpace(pos_start);
			auto wbox = p_Resource->getBoundingBox();
			cocos2d::Rect box(0, 0, p_Resource->getContentSize().width, p_Resource->getContentSize().height);
			if (box.containsPoint(node_pos))
			{
				OnTouchBeginObject();
			}
		}
		void wBasicObject::onTouchesMoved(const std::vector<Touch*>& touches, Event *unused_event)
		{
			auto pos_start = touches[0]->getPreviousLocation();
			Vec2 node_pos = p_Resource->convertToNodeSpace(pos_start);
			auto wbox = p_Resource->getBoundingBox();
			cocos2d::Rect box(0, 0, p_Resource->getContentSize().width, p_Resource->getContentSize().height);
			if (box.containsPoint(node_pos))
		...
		void wBasicObject::onTouchesEnded(const std::vector<Touch*>& touches, Event *unused_event)
		{
			auto pos_start = touches[0]->getLocation();
			Vec2 node_pos = p_Resource->convertToNodeSpace(pos_start);
			auto wbox = p_Resource->getBoundingBox();
			cocos2d::Rect box(0, 0, p_Resource->getContentSize().width, p_Resource->getContentSize().height);
			if (box.containsPoint(node_pos))
		...
		void wBasicObject::onTouchesCancelled(const std::vector<Touch*>&touches, Event *unused_event)
		{
			onTouchesEnded(touches, unused_event);
		}


		void wBasicObject::TouchEvent(cocos2d::Ref * ref, ui::Widget::TouchEventType touch_type)
		{
		...
		void wBasicObject::SetTouchEnable(bool value, bool use_all_at_once)
		{
			auto l = static_cast<ui::Layout*>(p_Resource);
			if (use_all_at_once)
			{
				if (value)
				{
					...
					_touchListener->onTouchesBegan = CC_CALLBACK_2(wBasicObject::onTouchesBegan, this);
					_touchListener->onTouchesMoved = CC_CALLBACK_2(wBasicObject::onTouchesMoved, this);
					_touchListener->onTouchesEnded = CC_CALLBACK_2(wBasicObject::onTouchesEnded, this);
					_touchListener->onTouchesCancelled = CC_CALLBACK_2(wBasicObject::onTouchesCancelled, this);
					l->getEventDispatcher()->addEventListenerWithSceneGraphPriority(_touchListener, l);
				}
				else
				{
					l->getEventDispatcher()->removeEventListener(_touchListener);
					CC_SAFE_RELEASE_NULL(_touchListener);
				}
			}
			else
			{
				if (value)
				{
					...
					l->addTouchEventListener(CC_CALLBACK_2(wBasicObject::TouchEvent, this));
				}
				else
				{
