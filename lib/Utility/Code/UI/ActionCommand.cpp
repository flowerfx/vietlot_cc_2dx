...
namespace Utility
{
	namespace UI_Widget
	{
		...
		void ActionCommand::ActionDetail::ClearHookCommand(const RKString &  name_value)
		{
			if (_hook_value)
			{
				delete _hook_value;
		...
		void ActionCommand::ActionDetail::SetValueHook( xml::BasicDec value, const RKString &  name_value_t)
		{
			if (!_hook_value)
			{
		...
		int ActionCommand::GetNumberDetailActInSquence(const RKString &  name_act, int idx_squence)
		{
		...
		int ActionCommand::GetNumberSquenceInAct(const RKString &  name_act)
		{
			if (p_list_actions.size() <= 0 || p_list_actions.find(name_act) == p_list_actions.end())
				return 0;
			auto list_act_detail_squence = p_list_actions.at(name_act);
		...
		ActionCommand::ActionDetail * ActionCommand::GetActionDetail(const RKString &  name_act, int idx_squence, int idx_detail)
		{
		...
		void ActionCommand::ActionDetail::ParseActionDetail(const JSON_VALUE value, const RKString &  name_value_t)
		{
			bool AddName = true;
			std::string name_value = name_value_t.GetString();
			if (name_value == "SIZE")
			...
			else if (name_value == "PARENT_STICK")
			{
				int IsStick = value[name_value].asInt();
				value_action->InsertDataInt(name_value, IsStick);
				AddName = false;
			...
			else if (name_value == "CONTENT_SIZE")
			{
		...
		FiniteTimeAction * ActionCommand::ActionDetail::GetAction(int & action_tag, cocos2d::Node * it_res, cocos2d::Node * parent, bool parent_stick, bool IsLayer)
		{
			...
			else if (name_action == "SET_SIZE")
			{
				...
				bool IsStick = value_action->GetDataInt("STICK") != 0;
				bool IsNinePath = value_action->GetDataInt("9SCALE") != 0;
				bool IsContentSize = value_action->GetDataInt("CONTENT_SIZE") != 0;
				size = Vec2(size.x * (GetGameSize().width / design_size.x),
					size.y * (IsStick ? (GetGameSize().height / design_size.y) : (GetGameSize().width / design_size.x)));
			...
			else if (name_action == "SET_TEXT")
			{
				...
				action_tag += 17;
				bool is_non_bitmap = (value_action->GetDataInt("NO_BITMAP") == 1);
				action = CallFunc::create([it_res, is_non_bitmap, value]()
				{
					if (is_non_bitmap)
					{
					...
					{
						static_cast<TextBMFont*>(it_res)->setString(LANG_STR(value).GetString());
					}
				});
			}
			...
			else if (name_action == "ROTATE_AROUND")
			{
				...
				if (_hook_value && _hook_value->HaveDatafloat("ANGLE_ROTATE_AROUND"))
				{
					begin_angle = _hook_value->GetDatafloat("ANGLE_ROTATE_AROUND");
				}
				Vec2 design_size = value_action->GetDataVector2("DESIGN");
				bool isForce = value_action->GetDataInt("FORCE") == 1;
				Vec2 point_rotate = Vec2(0, 0);
				if (isForce)
				{
					point_rotate = value;
				}
				else
				{
					if (parent_stick)
					{
						Vec2(value.x * GetGameSize().width / design_size.x, value.y * GetGameSize().height / design_size.y);
					}
					else
					{
						Vec2(value.x * GetGameSize().width / design_size.x, value.y * GetGameSize().width / design_size.x);
					}
				}
			...
			else if (name_action == "SET_POSITION")
			{
				...
				bool IsStick = parent_stick;
				if (value_action->HaveDataInt("PARENT_STICK"))
				{
					IsStick = value_action->GetDataInt("PARENT_STICK") != 0;
				}
				...
				if (force == 0)
				{
					pos = Vec2(value.x * (GetGameSize().width / design_size.x),
						value.y *( IsStick ? (GetGameSize().height / design_size.y): (GetGameSize().width / design_size.x)));
				}
				else
				{
					pos = value;
			...
			else if (name_action == "POSITION")
			{
				...
				Vec2 ratio = Vec2(GetGameSize().width / design_size.x, GetGameSize().height / design_size.y);
				if (style == 0)
				{
					Vec2 pos = Vec2(0, 0);
					if (force == 0)
					{
						pos = Vec2(value.x * ratio.x,
							value.y * (IsStick ? ratio.y : ratio.x));
					}
					else
					{
						pos = value;
				...
				else if (style == 1)
				{
					...
					if (force == 0)
					{
						pos = Vec2(delta_pos.x * ratio.x,
							delta_pos.y * (IsStick ? ratio.y : ratio.x));
					}
					else
					{
						pos = delta_pos;
		...
		{
			JSON_READER jreader;
			bool parsingSuccessful = JSON_READER_PARSE(jreader, jstr, JRootValue);

			if (!parsingSuccessful)
			{
				// report to the user the failure and their locations in the document.
				PINFO((std::string("Failed to parse configuration : ") + JSON_ERROR_CODE(jreader)).c_str());
				return false;
			}

			for (size_t i = 0; i < JSON_SIZE_VALUE(JRootValue); i++)
			{
		...
		ActionCommand::ActionDetail* ActionCommand::ParseDetailJsonAction(const JSON_VALUE value)
		{
		...
		ActionCommand::ActionRepeat * ActionCommand::ReadJsonAction(const JSON_VALUE value)
		{
		...
		Vector<FiniteTimeAction*> ActionCommand::GetSquenceActionDetail(const RKString &  name, int idx, int & action_tag, cocos2d::Node * it_res, cocos2d::Node * parent,bool parent_stick, bool IsLayer) const
		{
			...
			{
				auto action = list_action_detail.at(i)->GetAction(action_tag, it_res, parent,parent_stick, IsLayer);
				if (action)
				{
		...
		Vector<FiniteTimeAction*> ActionCommand::GetSequenceActionData(int & action_tag, cocos2d::Node * it_res, cocos2d::Node * parent, int & flag, bool parent_stick, bool IsLayer) const
		{
			//
			...
			for (auto it = p_list_actions.begin(); it != p_list_actions.end(); it++)
			{
				...
				{
					auto squence_action = GetSquenceActionDetail(it->first, i, action_tag, it_res, parent, parent_stick, IsLayer);
