...
namespace Utility
{
	namespace UI_Widget
	{
		...
		protected:
			...
			bool			p_scale_content;
			bool			p_mark_as_delete_resource;
			//state scale 9 //0 is x and y 1 is x 2 is y
			int				p_stage_scale9; 
			//custom design for the scale scale9 more correctly
			Vec2			p_custom_design;
            //min size scale
		...
		public:
			...
			virtual void InitParamParent(const RKString & name, xml::UILayerWidgetDec * xml_value, LayerEntity * layer_parent, WidgetEntity * parent_widget);
			...
			virtual SpriteFrame * GetFrameSource();
