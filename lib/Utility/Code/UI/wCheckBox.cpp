...
namespace Utility
{
	namespace UI_Widget
	{
		...
		/////////////////////////////////////////////////////////////
		void CheckBoxWidget::InitParam(const RKString &  name, xml::UILayerWidgetDec * xml_value)
		{
		...
		void CheckBoxWidget::InitTexture(xml::UILayerWidgetDec * xml_value)
		{
			...
			if (xml_value->GeneralValue->HaveDataInt("select"))
			{
				bool IsSelected = (xml_value->GeneralValue->GetDataInt("select") == 1);
				_widget->setSelected(IsSelected);
			}
		...
		bool CheckBoxWidget::IsSelected()
		{
			return static_cast<ui::CheckBox *>(p_Resource)->isSelected();
		}
	}
}