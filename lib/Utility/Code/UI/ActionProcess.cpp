...
#pragma region normal_method
		...
		void ActionProcess::SetScale(const cocos2d::Vec2 & s)
		{
		...
		void ActionProcess::SetSize(const cocos2d::Vec2 & s)
		{
			cocos2d::Vec2 p_Size(1, 1);
		...
		void ActionProcess::SetPosition(const cocos2d::Vec2 & p)
		{
		...
		void ActionProcess::SetOpacity(const RKUtils::BYTE & p)
		{
			GetResource()->setOpacity(p);
		}

		const RKUtils::BYTE & ActionProcess::GetOpacity()
		{
			return GetResource()->getOpacity();
		}

		void ActionProcess::SetColor(const Color3B & p)
		{
...
#pragma region action_process_method
		bool ActionProcess::SetActionProcess(const STATUS_MENU & stateFade, bool force_only)
		{
			//check
			if (force_only)
			{
		...
		void ActionProcess::ActionProcessFinish(const UI_STATE_FADE_ACTION & current_fd)
		{
		...
		Vector<FiniteTimeAction*> ActionProcess::CreateListActionFromSingleAction(const UI_STATE_FADE_ACTION & action_state, int loop, bool reserve, float time)
		{
		...
		Sequence * ActionProcess::CreateActionFromList(const Vector<ActionInterval*> & list_spawn_action)
		{
			Sequence * action = NULL;
		...
		void ActionProcess::SetResourceRunManyAction(const std::vector<Vector<FiniteTimeAction*>> & list_action_)
		{
			Action * action = NULL;
		...
		void ActionProcess::SetResourceRunAction(const UI_STATE_FADE_ACTION & action_state, int loop, bool reserve, float time)
		{
			Action * action = NULL;
...
#pragma region xml_load
		void ActionProcess::SetFirstValueXML(xml::UILayerWidgetDec * xml_value)
		{
			//set size value
		...
		void ActionProcess::SetValueFromXML(xml::UILayerWidgetDec * xml_value)
		{
			...
			//
			SetFirstValueXML(xml_value);
			//
			Color4B color = xml_value->OriginValue->GetDataColor("tint");
		...
		bool ActionProcess::SetActionCommandWithName(const RKString & name, CallFunc * func_callback)
		{
			...
			int flag = 0;
			auto action_squence = action_command->GetSequenceActionData(
				p_action_process, this->GetResource(),
				this->getUIWidgetType() == UI_TYPE::UI_LAYER ? this->GetParentLayer() : nullptr
				, flag, this->GetParentWidget() == nullptr ? true : this->GetParentWidget()->p_IsStickScreen,this->getUIWidgetType() == UI_TYPE::UI_LAYER);
		...
		bool ActionProcess::SetHookCommand(const RKString & name_command,const RKString & idx_act, int idx_squence, int idx_detail, const xml::BasicDec & value, const RKString & name_value)
		{
		...
		bool ActionProcess::ClearHookCommand(const RKString & name_command, const RKString & idx_act, int idx_squence, int idx_detail)
		{
			if (p_list_command.size() < 0)
			{
				return false;
		...
		void ActionProcess::InitParam(const RKString & name, xml::UILayerWidgetDec * xml_value)
		{
			p_Name = name;
...
#pragma region normal_function
		...
		Vec2 ActionProcess::PositionWithVectorUnit(const Vec2 & vec_unit)
		{
		...
		const Vec2  ActionProcess::GetPosition_TopLeft()
		{
			return PositionWithVectorUnit(Vec2(0, 1));
		}
		
		const Vec2  ActionProcess::GetPosition_TopRight()
		{
			return PositionWithVectorUnit(Vec2(1, 1));
		}
		
		const Vec2  ActionProcess::GetPosition_BottomLeft()
		{
			return PositionWithVectorUnit(Vec2(0, 0));
		}
		
		const Vec2  ActionProcess::GetPosition_BottomRight()
		{
			return PositionWithVectorUnit(Vec2(1, 0));
		}
		
		const Vec2  ActionProcess::GetPosition_Middle()
		{
			return PositionWithVectorUnit(Vec2(0.5, 0.5));
		}

		Vec2  ActionProcess::GetAnchorPoint()
		{
			return GetResource()->getAnchorPoint();
		}
		void ActionProcess::SetAnchorPoint(const Vec2 & p)
		{
			GetResource()->setAnchorPoint(p);
		}
		...
		void ActionProcess::SetName(const RKString & name)
		{
			p_Name = name;
			if (GetResource())
			{
		...
		const Vec2  ActionProcess::GetOriginPos() { return p_origin_value.GetDataVector2("pos"); }
		const Vec2  ActionProcess::GetFirstPos() { return p_first_value.GetDataVector2("pos"); }
		void ActionProcess::SetFirstPos(const cocos2d::Vec2 & p, bool force) {
		...
		//
		const RKUtils::BYTE  ActionProcess::GetOriginAlpha() { return (RKUtils::BYTE)p_origin_value.GetDataInt("alpha"); }
		void ActionProcess::SetOriginAlpha(const RKUtils::BYTE & p, bool force) {
		...
		const RKUtils::BYTE  ActionProcess::GetFirstAlpha() { return (RKUtils::BYTE)p_first_value.GetDataInt("alpha"); }
		void ActionProcess::SetFirstAlpha(const RKUtils::BYTE & p, bool force) {
		...
		//
		const Color3B  ActionProcess::GetOriginColor() {
			auto t = p_origin_value.GetDataColor("tint");
			return Color3B(t.r, t.g, t.b);
		}
		void ActionProcess::SetOriginColor(const Color3B & p, bool force) {
		...
		const Color3B  ActionProcess::GetFirstColor() {
			auto t = p_first_value.GetDataColor("tint");
			return Color3B(t.r, t.g, t.b);
		}
		void ActionProcess::SetFirstColor(const Color3B & p, bool force ) {
		...
		void  ActionProcess::SetFirstSize(const Vec2 & p, bool force) {
			if (force)
			{
				ActionProcess::SetSize(p);
			}
