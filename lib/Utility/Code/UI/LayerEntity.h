...
namespace Utility
{
	namespace UI_Widget
	{
		...
		protected:
			...
			virtual void OnSelectRadioBtn(cocos2d::ui::RadioButton* sender,int idx_select, cocos2d::ui::RadioButtonGroup::EventType type, WidgetEntity* _widget);
		public:

			Widget * GetWidgetElementByName(const RKString & name);
			WidgetEntity * GetWidgetEntityByName(const RKString & name);
		

			void AddLayerEntity(LayerEntity * layer);
			void AddWidgetEntity(WidgetEntity * layer);
			bool RemoveWidgetEntity(WidgetEntity * layer);
			bool RemoveWigetEnityByName(const RKString & name);
			...
			//hook the register and unregister
			template<class T>
			void OverloadRegisterUIWidget(xml::TYPE_UI_WIDGET type)
			{
				p_factory_ui_widget.RemoveProduct(type);
				p_factory_ui_widget.RegisterProduct<T>(type);
			}
		};
	}
}
