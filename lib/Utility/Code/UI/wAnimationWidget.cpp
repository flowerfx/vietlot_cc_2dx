...
namespace Utility
{
	namespace UI_Widget
	{
		...
		/////////////////////////////////////////////////////////////
		void AnimWidget::InitParam(const RKString &  name, xml::UILayerWidgetDec * xml_value)
		{
		...
		void AnimWidget::InitTexture(xml::UILayerWidgetDec * xml_value)
		{
			...
			{
				RKString str_sep = "_";
				if (xml_value->GeneralValue->HaveDataChar("split"))
				{
					str_sep = xml_value->GeneralValue->GetDataChar("split");
				}
				_list_frame_name = RKString(source_texture).Split(str_sep.GetString());
		...
		void AnimWidget::RunAnim(CallFunc * call_back)
		{
			...
			{
				if (static_cast<Sprite*>(GetResource())->getActionManager()->getNumberOfRunningActionsInTarget(GetResource()) > 0)
				{
					static_cast<Sprite*>(GetResource())->getActionManager()->resumeTarget(GetResource());
				}
				else
				{
