...
namespace Utility
{
	namespace UI_Widget
	{
		...
		/////////////////////////////////////////////////////////////
		void ButtonWidget::InitParam(const RKString &  name, xml::UILayerWidgetDec * xml_value)
		{
		...
		void ButtonWidget::InitTexture(xml::UILayerWidgetDec * xml_value)
		{
			...
			bool IsScale9Enable = p_isScale9Enable;
		...
		void ButtonWidget::SetColor(const Color3B & p)
		{
		...
		void ButtonWidget::SetSize(const Vec2 & s)
		{
			if (!p_is_only_text)
			{
		...
		void ButtonWidget::SetTitle(const RKString &  title)
		{
			p_title_text = title;
		...
		void ButtonWidget::SetActionBtn(std::function<void(void)> handle)
		{
			this->SetCallBack([this, handle](cocos2d::Ref *, ui::Widget::TouchEventType touch_type) {

				if (touch_type == ui::Widget::TouchEventType::BEGAN)
				{
					if (this->GetIdSound())
					{
						GetSound->PlaySoundEffect(this->GetIdSound()->id_click, this->GetIdSound()->delayTime, this->GetIdSound()->loop);
					}
				}
