...
namespace Utility
{
	namespace UI_Widget
	{
		...
		ListViewTableWidget::ListViewTableWidget()
		{
			...
			_state_drag_cell = 0;
			_on_end_drag = false;
			_state_drag_first_end = EventType_DragCell::DRAG_DOWN;
			_delta_pos = Vec2(0, 0);
		...
		/////////////////////////////////////////////////////////////
		void ListViewTableWidget::InitParam(const RKString & name, xml::UILayerWidgetDec * xml_value)
		{
		...
		void ListViewTableWidget::InitTexture(xml::UILayerWidgetDec * xml_value)
		{
			...
			else if (background_type == 3)
			{
				...
				panel_layout->setBackGroundImageScale9Enabled(p_isScale9Enable);
		...
		void ListViewTableWidget::InitTable()
		{
			...
			//stick screen
			bool isstick = xml_value->GeneralValue->GetDataInt("stick_screen") == 1;
			Vec2 ratio = Vec2(GetGameSize().width / xml_value->p_menu_widget->DesignSize.x, GetGameSize().height / xml_value->p_menu_widget->DesignSize.y);


			//create table
			Vec2 inner_size = xml_value->GeneralValue->GetDataVector2("inner_size");
			inner_size.x = inner_size.x * ratio.x;
			inner_size.y = inner_size.y * (isstick ? ratio.y : ratio.x);
			...
			p_size_child.x = p_size_child.x * ratio.x;
			p_size_child.y = p_size_child.y * (isstick ? ratio.y : ratio.x);
			...
			panel->SetCircleScroll((xml_value->GeneralValue->GetDataInt("circle_scroll") == 1) ? true : false);
			p_table = panel;
		...
		TableViewCell* ListViewTableWidget::tableCellAtIndex(TableView *table, ssize_t idx)
		{
			...
			if (!cell) {
				...
				if (resource_refer)
				{
					...
					resource_refer->GetResource()->retain();
		...
		void ListViewTableWidget::SetNumberSizeOfCell(size_t number, bool reload_)
		{
			...
			if ((number == number_size_of_cell) && reload_ == false)
				return;
		...
		{
			if (idx < 0 || idx >= number_size_of_cell)
				return;

			Vec2 offset_to = p_table->getOffsetFromIndex(idx);

			p_table->setContentOffset(-offset_to, true);
		}

		void ListViewTableWidget::ScrollToItemIdxLoop(int idx, float time_to_idx, int number_loop, float time_each_loop)
		{
			if (idx < 0 || idx >= number_size_of_cell)
				return;
			...
			p_table->actionScrollInDuration(-offset_to, time_to_idx, number_loop, time_each_loop);
		}

		bool ListViewTableWidget::IsIndexInIndiceList(int idx)
		{
			return p_table->IsIndexInIndicesList(idx);		
		}
		...
		void ListViewTableWidget::ReloadText()
		{
			WidgetEntity::ReloadText();
			p_refer_child->ReloadText();
			for (auto w : p_list_widget)
			{
				w->ReloadText();
			}
		...
		void ListViewTableWidget::JumpToEnd()
		{
			p_table->setContentOffset(Vec2::ZERO, true);
		}

		void ListViewTableWidget::UpdateCellAtIdx(int idx)
		{
			p_table->updateCellAtIndex(idx);
		}

		Vec2 ListViewTableWidget::GetFirstDistanceMove()
		{
			return _first_distance_move;
		}
	}
}