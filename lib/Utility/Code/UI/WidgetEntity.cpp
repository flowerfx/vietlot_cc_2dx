...
namespace Utility
{
	namespace UI_Widget
	{
		WidgetEntity::WidgetEntity()
		{
			...
			p_scale_content = false;

			p_stage_scale9 = 0;
			p_custom_design = Vec2(0, 0);
			//p_xml_value = nullptr;
		...
		void WidgetEntity::CopyValue(WidgetEntity * data)
		{
			...
			p_scale_content = data->p_scale_content;
			p_stage_scale9 = data->p_stage_scale9;
			p_custom_design = data->p_custom_design;
			...
			if (data->p_Resource && data->p_Resource->getTouchSize().width > 0)
			{
				p_Resource->setTouchSize(data->p_Resource->getTouchSize());
				p_Resource->setTouchSizeAnchor(data->p_Resource->getTouchSizeAnchor());
			}
			this->p_xml_dec = data->p_xml_dec;
		}
		...
		{
			if (!Visible() && !p_update_when_invisible)
				return false;
		...
		void WidgetEntity::InitParamParent(const RKString & name, xml::UILayerWidgetDec * xml_value, LayerEntity * layer_parent, WidgetEntity * widget_parent)
		{
			...
			p_isScale9Enable = (xml_value->GeneralValue->GetDataInt("scale_9") == 1);
			p_scale_content = (xml_value->GeneralValue->GetDataInt("content_size") == 1);
			p_update_when_invisible = (xml_value->GeneralValue->GetDataInt("update_invisible") == 1);
		...
		WidgetEntity * WidgetEntity::GetWidgetChildByName(const RKString & name_path)
		{
	    ...
	    void WidgetEntity::SetPosition(const Vec2 & p)
		{
			if (HaveAligneRes())
			{
				...
				{
					Vec2 anchor_point = GetAnchorPoint();
		...
		void WidgetEntity::SetSize(const Vec2 & s)
		{
			//s.x = (int)(s.x + 0.5f);
			//s.y = (int)(s.y + 0.5f);
			...
			if (p_isScale9Enable)
			{
				...
				{
					Vec2 own_design = p_custom_design;
					if (own_design.x == 0)
					{
						own_design = this->GetMenuParent()->GetDesignSize();
					}

					Vec2 origin_size = GetResource()->getContentSize();
					float delta_scale = GetGameSize().width / own_design.x;
			...
			else if (p_scale_content)
			{
				p_Resource->ignoreContentAdaptWithSize(false);
				p_Resource->setContentSize(cocos2d::Size(s.x, s.y));
			}
			else
			{
		...
		void WidgetEntity::SetEnable(bool value)
		{
			if (p_Resource)
			{
				p_Resource->setEnabled(value);
		...
		void WidgetEntity::InitParam(const RKString & name, xml::UILayerWidgetDec * xml_value)
		{
			if (p_isScale9Enable && xml_value->GeneralValue->HaveDataInt("stage_scale9"))
			{
				...
				p_custom_design = xml_value->GeneralValue->GetDataVector2("design_scale9");
			}
			...
			if (xml_value->GeneralValue->HaveDataInt("swallow_touch"))
			{
				p_Resource->setTouchEnabled(true);
				p_Resource->setSwallowTouches(true);
			}
			...
			if (xml_value->GeneralValue->HaveDataVector2("touch_zone"))
			{
				...
				p_Resource->setTouchSize(cocos2d::Size(value_touch_zone.x, value_touch_zone.y));

				if (xml_value->GeneralValue->HaveDataVector2("touch_anchor"))
				{
					Vec2 touch_anchor = xml_value->GeneralValue->GetDataVector2("touch_anchor");
					p_Resource->setTouchSizeAnchor(touch_anchor);
				}
			}
		}
		...
		SpriteFrame * WidgetEntity::GetFrameSource()
		{
			if (p_texture_source != "")
			{
				return SpriteFrameCache::getInstance()->getSpriteFrameByName(p_texture_source.GetString());
			}
		}
