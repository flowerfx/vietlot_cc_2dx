...
namespace Utility
{
	namespace UI_Widget
	{
		...
		protected:
			...
			LayerEntity * InitTheChildElement(const RKString & str, LayerEntity * parent_layer, xml::UILayerWidgetDec* re);
			void InitMenuWidgetEntity(const RKString & str);
			void InitMenuWidgetEntityWithParam(const RKString & name , const Vec2 & design_size);
			...
			std::function<void(LayerEntity * root_layer)> _func_action;

			Vec2		p_touch_start;
			Vec2		p_current_pos_touch;
		protected:
		...
		public:
			...
			void OnCheckTouchEvent(const RKString & name, UI_TYPE type_widget, WidgetEntity * _widget);
			...
			virtual WidgetEntity * GetWidgetChildByName(const RKString & name_path);
			virtual LayerEntity	 * GetLayerChildByName(const RKString & name_str);

			//
			virtual bool OnCallProcessOfChild(const RKString & name_child_path, const RKString & command_str, int type = 0);
			...
			void SetChildInteractive(const RKString & name_path, bool value);
			...
			virtual void OnTouchEnable(bool value);
			virtual void OnUpdateTouchMenu();
			virtual void OnTouchMenuBegin(const cocos2d::Point & p);
			virtual void OnTouchMenuEnd(const cocos2d::Point & p);
			virtual void OnTouchMenuHold(const cocos2d::Point & p);
			...
			virtual void onTouchesBegan(const std::vector<Touch*>& touches, cocos2d::Event  *event)override;
			virtual void onTouchesMoved(const std::vector<Touch*>& touches, cocos2d::Event  *event)override;
			virtual void onTouchesEnded(const std::vector<Touch*>& touches, cocos2d::Event  *event)override;
			virtual void onTouchesCancelled(const std::vector<Touch*>& touches, cocos2d::Event  *event)override;
            
			void ReloadUI(const RKString & path);
