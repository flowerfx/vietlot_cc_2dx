	...
	namespace UI_Widget
	{
		...
		void Screen::OnUpdateTouchMenu()
		{
			...
			if (InputMgr->GetNumberTouch() > 0)
			{
				...
				if (InputMgr->GetStateTouchOnScreen() == Control::STATE_TOUCH::STATE_PRESS)
				{
					p_touch_start = p_current_pos_touch = p;
					OnTouchMenuBegin(p);
				}
				else if (InputMgr->GetStateTouchOnScreen() == Control::STATE_TOUCH::STATE_HOLD)
				{
					OnTouchMenuHold(p);
					p_current_pos_touch = p;
				}
			}
