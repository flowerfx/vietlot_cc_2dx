...
namespace Utility
{
	namespace UI_Widget
	{
		...
		void LayerEntity::AddWidgetWithName(Widget * element, const RKString & name)
		{
			if (element)
			{
		...
		Widget * LayerEntity::GetWidgetElementByName(const RKString & name)
		{
			return static_cast<Widget*>(getChildByName(name.GetString()));
		}

		WidgetEntity * LayerEntity::GetWidgetEntityByName(const RKString & name)
		{
		...
		bool LayerEntity::RemoveWigetEnityByName(const RKString & name)
		{
			if (name != "")
			{
				int idx_layer = -1;
		...
		{
			if (!p_Visible && !p_update_when_invisible)
				return false;
		...
		void LayerEntity::AddEventType(xml::TYPE_UI_WIDGET type, WidgetEntity * wEntity, xml::UILayerWidgetDec* _widget)
		{
			switch (type)
			{
				...
				{
					static_cast<RadioButtonGroup*>(wEntity->GetResource())->addEventListener(
						[this, wEntity](RadioButton* radioButton, int index, cocos2d::ui::RadioButtonGroup::EventType type)
					{
						this->OnSelectRadioBtn(radioButton, index, type, wEntity);
					});
					break;
				}
			}
		...
		WidgetEntity * LayerEntity::InitWidgetEntity(xml::UILayerWidgetDec * widget, LayerEntity * parent_layer, WidgetEntity * parent_widget)
		{
			if (widget)
			{
                ...
                if(widget->typeValue == xml::TYPE_UI_WIDGET::WIDGET_TEXT_FIELD)
                {
					...
					wEntity = p_factory_ui_widget.CreateNewProduct(xml::TYPE_UI_WIDGET::WIDGET_EDIT_BOX);
#else
					wEntity = p_factory_ui_widget.CreateNewProduct(xml::TYPE_UI_WIDGET::WIDGET_TEXT_FIELD);
#endif
                }
                else
                {
                ...
                if (wEntity)
                {
					...
					//wEntity->SetFirstValueXML(_widget);
                }
		...
		void LayerEntity::OnTextFieldEvent(cocos2d::Ref* sender, EventType_TextField type, WidgetEntity* _widget)
		{
			...
			{
				static_cast<TextFieldWidget*>(_widget)->ClearPlaceHolder();
			...
			{
				static_cast<TextFieldWidget*>(_widget)->ResetPlaceHolder();
				if (_widget->IsInteractive())
				{
					if (menu_point_to)
					{
			...
			case EventType_TextField::ON_ENTER_BTN:
			{
				if (_widget->IsInteractive())
				{
					if (menu_point_to)
					{
						func_call_widget(*menu_point_to, _widget->GetName() + "_ENTER_BTN", _widget->getUIWidgetType(), _widget);
					}
				}
				break;
			}
		...
		void LayerEntity::OnSelectRadioBtn(cocos2d::ui::RadioButton* sender,int idx_select, cocos2d::ui::RadioButtonGroup::EventType type, WidgetEntity* _widget)
		{
			switch (type)
			{
				case cocos2d::ui::RadioButtonGroup::EventType::SELECT_CHANGED:
				{
					if (_widget->IsInteractive())
					{
						if (menu_point_to)
						{
							...
							func_call_widget(*menu_point_to, _widget->GetName() + "_" + std::to_string(idx_select), _widget->getUIWidgetType(), _widget);
						}
					}
		...
		WidgetEntity * LayerEntity::GetWidgetChildByName(const RKString & name_path)
		{
		...
		LayerEntity	 * LayerEntity::GetLayerChildByName(const RKString & name_path)
		{
			auto name_list = name_path.Split(".");
		...
		void LayerEntity::RegisterUIWidget()
		{
			...
			p_factory_ui_widget.RegisterProduct<TextFieldHandleWidget>(xml::TYPE_UI_WIDGET::WIDGET_TEXT_FIELD);
