...
//        Utility::IAP::IAPManager::GetInstance()->SetLastTransactionID()
        ...
        if(atoi(version.substr(0, 1).c_str()) > 7)
        {
            ...
            if (receipt)
            {
                NSLog(@"receipt not null");
                NSString *base64Encoded = [receipt base64EncodedStringWithOptions:0];
                last_transaction =[base64Encoded UTF8String];
        ...
        Director::getInstance()->getScheduler()->performFunctionInCocosThread([last_transaction](){
            Utility::IAP::handle_iap_success(last_transaction);
        });
