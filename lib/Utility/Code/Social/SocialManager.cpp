...
namespace Utility
{
	namespace Social
    {
        ...
        void SocialManager::Init()
        {
         ...
         void SocialManager::DownloadFile(RKString url, RKString name_save, const std::function<void(void * data, void * str, int tag)> & call_back, bool isSquence)
        {
			CCLOG("Download url: %s", url.GetString());
			if (isSquence)
			{
				p_downloader->OnDownloadObject(url, name_save, call_back);
			}
			else
			{
				p_downloader->OnDownloadObjectparallel(url, name_save, call_back);
			}
