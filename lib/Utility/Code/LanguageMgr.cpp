...
namespace Utility
{
	namespace Language
	{
		...
		void Language::InsertString(const RKString &  id,  RKString str)
		{
			if (str.Contains("\\n"))
			{
				str.ReplaceAll("\\n", "\n");
		...
		bool Language::SetNewLanguage(LANGUAGE_TYPE lang)
		{
			...
			else
			{
				...
				bool res = cocos2d::FileUtils::getInstance()->GetDataFromZipFile((DataDir() + path_language).GetString(), buffer, &lSize);
				if (res)
				{
					RKString str_lang = (char*)buffer;
		...
		bool Language::Find_Str_With_Id(const RKString &  id, RKString & str)
		{
			char first_char_id = std::string(id.GetString())[0];
		...
		RKString   Language::GetString(const RKString &  id)
		{
			RKString str = "";
