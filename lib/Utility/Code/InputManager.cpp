...
namespace Utility
{
	...
	namespace Control
	{
		...
		{
			auto it = m_TouchList.find(idx);
			if (it == m_TouchList.end() || m_TouchList.size() <= 0)
			{
				return nullptr;
			}
			else
		...
		{
			auto it = m_TouchReleaseList.find(idx);
			if (it == m_TouchReleaseList.end() || m_TouchReleaseList.size() <= 0)
			{
				return nullptr;
			}
			else
		...
		bool InputManager::IsPressOnBound(cocos2d::Rect re)
		{
			if (m_TouchList.size() <= 0)
				return false;
			auto it = m_TouchList.begin();

			if (re.containsPoint(it->second->pos))
				return true;
			return false;
		}
		...
		{
			if (m_TouchReleaseList.size() <= 0)
				return false;
			auto it = m_TouchReleaseList.begin();
