...
#ifdef USE_ZIP
XMLError XMLDocument::LoadFileZip(const char* filename)
{
	void * data = nullptr;
	long size = 0;
	bool res = cocos2d::FileUtils::getInstance()->GetDataFromZipFile(filename, data, &size);
	if (!res)
	{
	...
	_charBuffer = new char[size + 1];
	memcpy(_charBuffer, data, size);//RKFile_Read(fp, _charBuffer, size);
