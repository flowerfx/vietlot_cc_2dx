...
#define STR_SAVE_GG_STATE		"GGState"
#define STR_SAVE_RECENT_CHAT	"RecentChat"
...
{
public:
    static const std::string TYPE_FACEBOOK;
    static const std::string TYPE_GOOGLE;
    static const std::string TYPE_SDKLIB;
    static const std::string TYPE_CHANGE_SERVER;
    static const std::string TYPE_APPLEID;
private:
	...
	Platform::AppInfo*				_app_info;
	Platform::DeviceInfo*			_device_info;
...
public:
    ...
    void Network_CheckEmailExist(RKUtils::RKString email);
	...
	void HandleURLImgDownload(RKString url, RKUtils::RKString name, const std::function<void(void * data, void * str, int tag)> & func , bool isSequence = true);
	void HandleAvatarDownload(std::string url, const std::function<void(void * data, void * str, int tag)> & func);

	Sprite * CreateAvatarFromImg(void * img ,void * str , cocos2d::Texture2D::PixelFormat fmat = cocos2d::Texture2D::PixelFormat::RGB5A1, bool save_cache = true);
private:
    cocos2d::Map<std::string, Texture2D*> p_list_texture_avatar_download;

	//the pixel have bigger circle will have alpha = 0;
	void MakeImage2DCircle(Image * img, cocos2d::Texture2D::PixelFormat fmat);
public:
	...
	void clearAvatarUser();

	Platform::AppInfo* GetAppInfo();
	Platform::DeviceInfo* GetDeviceInfo();
...
#define APPINFO GameMgr->GetAppInfo()
#define DEVICEINFO GameMgr->GetDeviceInfo()
#define OPEN_URL(url) Platform::showWeb(url)
#define PLAY_MUSIC_SE GameMgr->PlayMusicSequence()
