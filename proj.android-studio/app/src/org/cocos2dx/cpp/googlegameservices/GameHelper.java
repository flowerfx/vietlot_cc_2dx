...
//import vn.me.engine.util.Utility;
...
//        }
    ...
    public void setup(GameHelperListener listener) {
        ...
        mGoogleApiClient = mGoogleApiClientBuilder.addConnectionCallbacks(this)
    ...
    public void onStart(Activity act) {
        ...
        if (mConnectOnStart) {
            if (mGoogleApiClient.isConnected()) {
                debugLog("GameHelper: client was already connected on onStart()");
            } else {
...
//        }
    ...
    public String getToken(IActionListener callback) throws Exception{
    	this.callback = callback;
    	AsyncTask<Void, Void, String> task = new AsyncTask<Void, Void, String>() {
    		String token = null;
			@Override
			protected String doInBackground(Void... params) {
                debugLog("GameHelper --> GoogleAuthUtil --> doInBackground ");
					String username = "";
					String scope = "oauth2:https://www.googleapis.com/auth/userinfo.profile";
			        try {
			            //username = Plus.AccountApi.getAccountName(mGoogleApiClient);
			            //username = Games.Players.getCurrentPlayer(mGoogleApiClient).getName();
			            username = Games.getCurrentAccountName(mGoogleApiClient);
                        debugLog("GameHelper --> GoogleAuthUtil --> username " + username);
			        } catch (SecurityException e)
			        {
                        debugLog("GameHelper --> GoogleAuthUtil --> SecurityException " + e);
			            return "ERROR:Google Plus Security Exception, need permissions.";
			        }
			 
			        try {
			        	if (mActivity != null && 
			        			!Utility.isNullOrEmpty(username) && 
			        			!Utility.isNullOrEmpty(scope)) {
			        		token = GoogleAuthUtil.getToken(mActivity, username, scope);
                            debugLog("GameHelper --> GoogleAuthUtil --> token " + token);
						}
			            //token = username + "|" + GoogleAuthUtil.getToken(mActivity, username, scope);
			        	//token = GoogleAuthUtil.getToken(mActivity, username, scope);
			            if (token != null) {
				        	GameHelper.this.callback.actionPerformed(token);

			            }
			        } catch (GooglePlayServicesAvailabilityException e) {
			            token = "ERROR:GooglePlayServiceAvailability: " + e.getConnectionStatusCode();
			        } catch (UserRecoverableAuthException e)
			        {
			            token = "ERROR:Recoverable, retrying.";
			            mActivity.startActivityForResult(e.getIntent(), RC_RESOLVE);
			        } catch (IOException e) {
			            token = "ERROR:IO Exception, try again later: " + e.getLocalizedMessage();
			        } catch (GoogleAuthException e) {
			            token = "ERROR:Auth exception, total fail: " + e.getLocalizedMessage();
			        }
			        return token;
			}
    ...
    public MGoogleAccount getProfileInformation() {
    	
		MGoogleAccount account = null;
		try {
            debugLog("GameHelper --> GoogleAuthUtil isConnected " + mGoogleApiClient.isConnected() + " is null " + (Games.Players.getCurrentPlayer(mGoogleApiClient) != null));
			if (mGoogleApiClient.isConnected()
					&& Games.Players.getCurrentPlayer(mGoogleApiClient) != null) {
				Player p = Games.Players.getCurrentPlayer(mGoogleApiClient);
				account = new MGoogleAccount();
				account.email = Games.getCurrentAccountName(mGoogleApiClient);
				account.gender = null;
				account.given_name = p.getDisplayName();
				account.id = p.getPlayerId();
				account.link = null;
				account.locale = null;
				account.name = p.getDisplayName();
				account.picture = p.getIconImageUrl();
                debugLog("GameHelper --> GoogleAuthUtil isConnected --> play email " + account.email);
			} else {
			...
			/*if (mGoogleApiClient.isConnected()
				...
				account.email = Plus.AccountApi
						.getAccountName(mGoogleApiClient);
				account.gender = p.getGender() == Gender.MALE ? "male" : (p
						.getGender() == Gender.FEMALE ? "female" : "other");
				account.given_name = p.getDisplayName();
				account.id = p.getId();
				account.link = p.getUrl();
				account.locale = p.getLanguage();
				account.name = p.getName().getFormatted();
				account.picture = p.getImage().getUrl();
			} else {
				Log.i(TAG, "Person information is null");
			}*/
		} catch (Exception e) {
		...
		Log.i(TAG, account != null ? account.toString() : "null");
		return account;
	}
}